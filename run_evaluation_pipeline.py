...
def main():
    parser = argparse.ArgumentParser(
        description="動画評価パイプライン統合実行スクリプト",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  python run_evaluation_pipeline.py --real real_0804.mp4 --gen Receiver_0804.mp4
  python run_evaluation_pipeline.py --real real_0804.mp4 --gen Receiver_0804.mp4 --skip-models
        """
    )
    ...
    try:
        ...
        # ==========================================
        run_command(
            f"python preprocessing/preprocess.py --real {args.real} --gen {args.gen}",
        ...
        result = run_command(
            f"python evaluation/compute_dtw_min_diff_improved.py --real features_tmp/real.npy --gen features_tmp/gen.npy --min_shift {args.min_shift} --max_shift {args.max_shift} --remove_invalid",
            "5. DTWシフト値の算出（改良版）"
        )
        
        # DTWの結果からシフト値を抽出（簡単な実装のため、ログから読み取り）
        ...
        run_command(
            f"python preprocessing/shift_videos_trim.py --real {args.real} --gen {args.gen} --shift {shift_value} --fps {args.fps} --out-real {shifted_real} --out-gen {shifted_gen}",
            "6. 動画のシフト処理"
        )
        ...
        # ==========================================
        run_command(
            f"python preprocessing/preprocess.py --real {shifted_real} --gen {shifted_gen}",
            "7. シフト後のアライン済みフレーム抽出"
        )
        ...
        run_command(
            'python evaluation/compute_dscore.py --detectors detectors.pkl --real "frames/aligned/real/*.png" --gen "frames/aligned/gen/*.png"',
            "18. D-Score計算（3分程度）",
            check=False  # detectors.pklがない場合はスキップ
        )
